...
<div class="mt-6 mb-2 px-3" x-show="sidebarOpen" x-transition.opacity>
    <p class="text-xs font-semibold text-emerald-500 uppercase tracking-wider">Estoque</p>
</div>
...
{{-- Seção: Empréstimos --}}
<div class="mt-6 mb-2 px-3" x-show="sidebarOpen" x-transition.opacity>
    <p class="text-xs font-semibold text-emerald-500 uppercase tracking-wider">Emprestimos</p>
</div>
...
<a href="{{ route('inventory.index') }}"
   class="flex items-center px-3 py-2.5 rounded-lg text-sm font-medium transition
          {{ str_starts_with($currentRoute, 'inventory.')
              ? 'bg-emerald-700 text-white'
              : 'text-emerald-200 hover:bg-emerald-800 hover:text-white' }}">
    <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <span class="ml-3 whitespace-nowrap" x-show="sidebarOpen" x-transition.opacity>Inventário</span>
</a>

{{-- Seção: Administração (apenas admin) --}}
@if($user->isAdmin())
    <div class="mt-6 mb-2 px-3" x-show="sidebarOpen" x-transition.opacity>
        <p class="text-xs font-semibold text-emerald-500 uppercase tracking-wider">Administracao</p>
    </div>
